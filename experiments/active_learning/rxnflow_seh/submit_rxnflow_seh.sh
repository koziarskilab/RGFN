#!/bin/bash
#SBATCH --job-name=al_rxnflow_seh
#SBATCH --time=05:00:00                       # GFN training dominates; docking ~1s/mol (GPU oracle). Mirrors al_seh / rxnflow_6td3.
...
# RxnFlow (synthesizable baseline) active-learning loop on sEH — the head-to-head
# synthesizable peer for the RGFN sEH run (experiments/active_learning/seh/submit_al_seh.sh).
# IDENTICAL oracle (docking_seh) / seed / budget / β / proxy M; only the generator differs
# (RxnFlow's reaction-template + building-block synthesis GFlowNet, carries a route ->
# has_route=1). sEH is the classic GFlowNet benchmark target: a reproduction check before 6TD3.
#
...
# two envs are separate processes and don't need to agree.
#
# Submit with:  sbatch experiments/active_learning/rxnflow_seh/submit_rxnflow_seh.sh

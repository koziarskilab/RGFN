...
# SCENT (cost-aware baseline) active-learning loop on 6TD3 — the cost-aware peer to
# the RGFN run (experiments/active_learning/6td3/submit_al_6td3_gpu.sh) and the
# FragGFN/RxnFlow baselines. IDENTICAL oracle / seed / budget / β / proxy; only the
# generator differs (SCENT's cost-guided reaction-GFN on the SMALL library with its
# shipped prices+yields). Synthesizable -> has_route=1 + routes.jsonl.
#
...
#
# Submit with:  sbatch experiments/active_learning/scent_6td3/submit_scent_6td3.sh

#!/bin/bash
#SBATCH --job-name=fr_rgfn_clpp
#SBATCH --time=24:00:00                         # ClpP single-target docking-in-loop: ~16h at 400 iters (benchmark 69691)
...
# Fixed-reward (single-shot) RGFN on ClpP GPU docking, built from the shared SMALL library
# (glue_standard_v1). OracleRewardProxy(DockingClpPOracle) is the per-step reward; empty_cache
# before each dock keeps QuickVina2-GPU alive during training (the guard the raw
# DockingMoleculeProxy lacked -> job 69567 failure). See configs/glue/fixed_reward_clpp.gin.
# Submit: sbatch experiments/fixed_reward/clpp/submit_fixed_clpp.sh
set -uo pipefail
...
python scripts/fixed_reward.py \
        --cfg configs/glue/fixed_reward_clpp.gin \
        --seed 42 --root-dir "$FR_ROOT_DIR"
echo "[fr_rgfn_clpp] done"

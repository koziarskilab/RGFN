...
| [020](../Logs/020_stdlib-fourway-seh-fixed-reward.md) | 2026-07-02 | sEH — same-library four-way fixed-reward (RGFN/SCENT/RxnFlow share `glue_standard_v1`; FragGFN native foil) | **Machinery built + validated; runs queued.** New `glue.chemistry.ChemLibrary` + `GlueReactionDataFactory` + canonical `data/libraries/glue_standard_v1` (SCENT's SMALL set: 418 frags/112 rxns, priced). Removes the library confound in entry `019`: RGFN(69615, 3-day walltime — larger library ~2× slower/iter)/RxnFlow(69614) re-run on the shared library (SCENT 69608 already on it; FragGFN 69606 the foil). Smoke tests pass — RGFN builds routed mols from the shared set, RxnFlow trains NaN-free on the shrunk 412-block env. Four-way AiZynth+cost table pending |
...
| [021](../Logs/021_fixed-reward-docking-matrix.md) | 2026-07-03 | Fixed-reward 4-reward × 4-generator matrix — per-step GPU docking for EVERY generator (ClpP + 6TD3 differential) on the SMALL library | **Mechanism validated; matrix built, not yet launched.** Benchmark (job 69691) showed cross-env per-step docking amortizes to ~10-13% overhead (sEH 1.4 s/mol, 6TD3 0.95 s/mol at batch 100) → per-step `score_batch.py` bridge feasible for a single run, no persistent server, 400-iter cap. New reusable `glue.proxies.OracleRewardProxy` (any `GlueOracle`→GFN reward, `clip(-raw/norm)` + per-step `empty_cache` so QV2-GPU isn't starved during training — the guard job 69567 lacked), `DockingClpPOracle`, and baseline `DockingBridgeReward`/`DockingBridgeProxy`. Two smoke jobs passed end-to-end: RGFN in-env 6TD3 (69692; loss 175→90, 100% GPU free before sampling, routed candidates) + FragGFN cross-env (69693; 100% per-step dock success while torch trains). Launch: `bash experiments/fixed_reward/submit_matrix.sh` |
